...
INFILE=$(echo "$INFILE" | sed "s/\_\([a-zA-Z0-9]*\)\_/<i>\1<\/i>/g"); # replace * with <i>
INFILE="$(echo "$INFILE" | sed -e 's/\!\[\(.*\)\](\(.*\))/<img src=\2 alt=\1>/g')"; # markdown url
INFILE="$(echo "$INFILE" | sed -e 's/\[\(.*\)\](\(.*\))/<a href=\2>\1<\/a>/g')"; # markdown url
INFILE="$(echo "$INFILE" | sed -e "s|[[:space:]]http[:]//[^ ]*[a-zA-Z][[:space:]]|<a href=\"\0\">\0</a>|g")"; # replace urls with html urls
...
sed -i "s/<time>\(.*\)<\/time>/<time>Modified: $(date +'%A, %B %d %Y - %I:%M:%S %p')<\/time>/" index.html;
