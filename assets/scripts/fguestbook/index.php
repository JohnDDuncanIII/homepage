...
// START CONFIGURATION
$database = 'entries'; // Your TXT file name as database.
$per_page = 5; // The number of items you want to display per page.
$time_zone = 'America/New_York'; // Look at `date_default_timezone_set()`
    ...
    for($i = 0; $i < count($data); $i++) {
        ...
        if(isset($_GET['guest']) && preg_match('/[0-9]+/', $_GET['guest'])) {
            if($item[3] == $_GET['guest']) {
                ...
                echo "<time datetime=\"" . date('c', $item[3]) . "\">" . date('Y/m/d g:ia', $item[3]) . "</time>";
